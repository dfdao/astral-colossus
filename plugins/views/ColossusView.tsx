...
export function ColossusView(): JSX.Element {
  ...
  const isFindable = (planetDetails: any, currentBlockNumber: number) => {
    ...
    return (
      prospectedBlockNumber !== 0 && !hasTriedFindingArtifact
  ...
  const processAndReturnPlanets = async (
    ...
    const prevScore = (
      await daoPlayer.contributions(wallet.address)
    ).toNumber();
    ...
    let numReturned = 0;
    try {
      const processTx = await daoPlayer.processAndReturnPlanets(
        locationIds,
        findArgsList,
        { gasLimit }
      );
